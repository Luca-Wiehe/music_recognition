...
def train_model(model, train_data, val_data, hparams, device, collate_fn, loss_func=torch.nn.CTCLoss(blank=0), epochs=20, scheduler_state=None):
    """
    ...
    Parameters:
        ...
        collate_fn (callable): Collate function for DataLoader batching.
    ...
    for i, epoch in enumerate(range(epochs)):
        ...
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=hparams["batch_size"], shuffle=True, collate_fn=collate_fn)
        ...
        val_loader = torch.utils.data.DataLoader(val_data, batch_size=hparams["batch_size"], shuffle=True, collate_fn=collate_fn)
